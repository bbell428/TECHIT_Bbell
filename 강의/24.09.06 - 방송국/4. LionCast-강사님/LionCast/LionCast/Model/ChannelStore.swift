...
//            print(html)
            ...
            let pragramsStrings = html.replacingOccurrences(of: "\n" , with:" ").subStringsBetween(
                "<li class=\"",
                "</li>")
            
            for pragramsString in pragramsStrings {
                let channelName: String = pragramsString.subStringBetween(
                    "<span class=\"channel\">",
                    "</span>")
                
                let programTitle: String = pragramsString.subStringBetween(
                    "<span class=\"tit\">",
                    "</span>")
                
                for index in 0..<channels.count {
                    let tempChannel = channels[index]
                    
                    if tempChannel.name == channelName {
                        channels[index].programName = "\(programTitle)"
                    }
                }
            }
        } catch {
